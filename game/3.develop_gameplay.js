const GameModule = () => {
  ...
  function drawBoard(newBoard, isGameOver = false) {
    ...
    processingElement.className = "lds-dual-ring";
    processingElement.id = "move-processing";
    ...
    newBoard.forEach((row, rowIndex) => {
      row.forEach((_, colIndex) => {
        ...
        cellElement.addEventListener("click", async function (e) {
          ...
          clickCount += 1;
          cellElement.appendChild(processingElement);
          ...
          if (cell._isFlagged) {
            ...
            try {
              ...
              processingElement.style.display = "none";
              grid.style.pointerEvents = "unset";
            }
            return;
          }
          ...
          try {
            ...
            if (res && res.receipt.logs[0].data && gameStatus !== -1) {
              drawBoard(newBoard);
              if (checkForWin()) {
                document.querySelector("#game-validate").style.display =
                  "block";
  ...
  function renderTableItem(item, index) {
    const isUserWallet =
      contractInteraction.WalletData.Wallet.address === item.player;

    const leaderboardTableEl = document.querySelector(
    ...
    <div class="leaderboard_table_data_name">${formatAddress(item.player)} ${
      isUserWallet ? "(You)" : ""
    }  </div>
